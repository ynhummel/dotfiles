...
if nix config show experimental-features 2>/dev/null | grep -qw "flakes"; then
  ...
  if [ -f /etc/nix/nix.conf ] && [ ! -L /etc/nix/nix.conf ]; then
    ...
    mv ~/.config/nix/nix.conf ~/.config/nix/nix.conf.bak
  fi

  mkdir -p ~/.config/nix
  sudo ln -sf "$(pwd)/nix-conf/etc/nix/nix.conf" ~/.config/nix/nix.conf
...
nix run github:nix-community/home-manager -- switch --flake ../nix#nana
